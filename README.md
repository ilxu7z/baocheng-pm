...
| **自动审查** | OCR 代码审查在研发主管（开发）完成后自动触发，结果不阻塞流程 |
...
| `ld-r` | 调度长 | 执行办·调度长 | 正一品 | 项目管理、全权执行调度、任务分解 | DeepSeek V4 |
| `guihua` | 军师 | 规划部·规划师 | 正二品 | 需求拆解 → 生成 TASK.md 方案 | DeepSeek V4 Pro |
| `shenyi` | 审计官 | 审议部·审议官 | 正一品 | 独立审查 TASK.md → 通过或驳回 | DeepSeek V4 Pro |
| `paifa` | 路由主管 | 人力路由处·人力路由负责人 | 正二品 | 任务路由 → 派发到执行部门 | DeepSeek V4 Flash |
| `wenan` | 内容主管 | 内容部·内容负责人 | 正二品 | 网站文案/品牌故事/SEO/翻译 | GLM-5.1 |
| `daima` | 研发主管 | 开发部·开发负责人 | 正二品 | 前端开发/功能实现/性能优化 | Claude / DeepSeek |
| `sheji` | 视觉主管 | 设计部·设计负责人 | 正二品 | 视觉规范/UI设计/图片生成 | Gemini |
| `shencha` | 品控官 | 质控部·质控负责人 | 正二品 | 独立质量验收/对照标准评分 | DeepSeek V4 |
| `huizong` | 交付主管 | 交付汇总处·交付负责人 | 正二品 | 整合交付/生成交付报告 | DeepSeek V4 Flash |
| `rongcui` | 运维主管 | 运维组·运维专员 | 正三品 | 晨间简报/团队状态日报 | DeepSeek V4 Flash |
...
| **自动触发** | 研发主管完成 → 任务进入 Review 状态时自动触发 |
...
| 文案（内容主管） | GLM-5.1 | 中文文案质量高 |
| 代码（研发主管） | Claude / DeepSeek V4 | 代码生成准确 |
| 视觉（视觉主管） | Gemini | 多模态理解强 |
...
6. → 自动触发 OCR 代码审查（研发主管完成 → Review 状态）
...
研发主管（开发部）完成代码产出后，任务状态推进到 `Review`，此时看板自动触发 OCR 审查：
...
看板 server.py 自动调用 ocr_auto_trigger
  ...
  │
  ▼
进入正常审计官流程（不阻塞，OCR 异常由 try/except 包裹）
```
